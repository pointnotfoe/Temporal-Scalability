...
char MVType;
int sad_p[37],sad_n[37];
int frameNum=0;//number of times set image is called
  ...
  m_iMBNumW (w / MBSIZE), m_iMBNumH (h / MBSIZE)
{
  ...
  //hierichal stuff
  for (i = 0;i < 16;i++) {
	  f [i]= new float [_w * _h * 3 / 2];// what is 3/2??
	  f_new[i] = new float [_w * _h * 3 / 2];
...
//MV buffer prev
  p_iMVy = new int *[m_iMBNumH];
  p_iMVy[0] = new int[m_iMBNumW * m_iMBNumH];
  for(i = 1; i < m_iMBNumH; i++) {
    p_iMVy[i] = p_iMVy[i-1] + m_iMBNumW;
  }

  p_iMVx = new int *[m_iMBNumH];
  p_iMVx[0] = new int[m_iMBNumW * m_iMBNumH];
  for(i = 1; i < m_iMBNumH; i++) {
    p_iMVx[i] = p_iMVx[i-1] + m_iMBNumW;
  }
  
  //MV buffer next
  n_iMVy = new int *[m_iMBNumH];
  n_iMVy[0] = new int[m_iMBNumW * m_iMBNumH];
  for(i = 1; i < m_iMBNumH; i++) {
    n_iMVy[i] = n_iMVy[i-1] + m_iMBNumW;
  }

  n_iMVx = new int *[m_iMBNumH];
  n_iMVx[0] = new int[m_iMBNumW * m_iMBNumH];
  for(i = 1; i < m_iMBNumH; i++) {
    n_iMVx[i] = n_iMVx[i-1] + m_iMBNumW;
  }
	...
	if (MVType == 'p') {
		for (y = 0; y < _h; y += BLOCKSIZE) {
			for (x = 0; x < _w; x += BLOCKSIZE) {
				GetBlockRecon(f, _ydataRef, y, x, p_iMVy[y / MBSIZE][x / MBSIZE], p_iMVx[y / MBSIZE][x / MBSIZE]);
			}
		}
		...
		for (y = 0; y < _h2; y += BLOCKSIZE) {
			for (x = 0; x < _w2; x += BLOCKSIZE) {
				GetBlockRecon(U, _udataRef, y, x, p_iMVy[y / MBSIZEUV][x / MBSIZEUV] / 2, p_iMVx[y / MBSIZEUV][x / MBSIZEUV] / 2);
			}
		}

		for (y = 0; y < _h2; y += BLOCKSIZE) {
			for (x = 0; x < _w2; x += BLOCKSIZE) {
				GetBlockRecon(V, _vdataRef, y, x, p_iMVy[y / MBSIZEUV][x / MBSIZEUV] / 2, p_iMVx[y / MBSIZEUV][x / MBSIZEUV] / 2);
			}
		}
	}if (MVType == 'n') {
		for (y = 0; y < _h; y += BLOCKSIZE) {
			for (x = 0; x < _w; x += BLOCKSIZE) {
				GetBlockRecon(f, _ydataRef, y, x, n_iMVy[y / MBSIZE][x / MBSIZE], n_iMVx[y / MBSIZE][x / MBSIZE]);
			}
		}
		...
		for (y = 0; y < _h2; y += BLOCKSIZE) {
			for (x = 0; x < _w2; x += BLOCKSIZE) {
				GetBlockRecon(U, _udataRef, y, x, n_iMVy[y / MBSIZEUV][x / MBSIZEUV] / 2, n_iMVx[y / MBSIZEUV][x / MBSIZEUV] / 2);
			}
		}

		for (y = 0; y < _h2; y += BLOCKSIZE) {
			for (x = 0; x < _w2; x += BLOCKSIZE) {
				GetBlockRecon(V, _vdataRef, y, x, n_iMVy[y / MBSIZEUV][x / MBSIZEUV] / 2, n_iMVx[y / MBSIZEUV][x / MBSIZEUV] / 2);
			}
		}
	}
	...
	unsigned char *pcBuf)
{
	...
	cout << "setting frames in setImage function" << endl;
  ...
  for (i = 0; i < len; i++) {
	...
	//  cout << "ydata " << _ydata[0][i] << endl;
	  f[frameNum][i] = (float)(pcBuf[i]);
  }
  
  //cout << "f " << frameNum<< "="<< f[frameNum] <<endl;

  pcBuf += len;
  for (i = 0; i < len / 4; i++) {
	  _udata[0][i] = (float)(pcBuf[i]);
	  U[frameNum][i] = (float)(pcBuf[i]);
    ...
    float fQstep)                    //quantization step size,
{
  ...
  if (!bIsIFrame) {
      ...
      GetPredError();*/
	  cout << "gonna do hierarchal" << endl;

	  hireacrical_prediction();
	  cout << "did hierarchal" << endl;
  ...
  for(y = 0; y < _h; y += BLOCKSIZE) {
      for(x = 0; x < _w; x += BLOCKSIZE) {
          codeBlock(f, y, x, fQstep);
		  cout << "did y code block" << endl;
  ...
  cout << "setting frame buffer pointer" << endl;
    ...
    for (i = -18; i <= 18; i++) {
       for (j = -18; j <= 18; j++) {
           ...
           if (iSAD < iSAD0 * 0.925f) {  
			...
			if(MVType == 'n'){
				n_iMVy[y / MBSIZE][x / MBSIZE] = iMVy;
				 n_iMVx[y / MBSIZE][x / MBSIZE] = iMVx;
	
	
	
}
			if(MVType == 'p'){
				p_iMVy[y / MBSIZE][x / MBSIZE] = iMVy;
				 p_iMVx[y / MBSIZE][x / MBSIZE] = iMVx;
...
void IEncoder::EncodeMV()
{
    ...
    for(y = 0; y < m_iMBNumH; y++) {
        for(x = 0; x < m_iMBNumW; x++) {
			if (MVType == 'n') {
				uiIndex = ForwardGolombRiceIndexMapping(n_iMVy[y][x]);
				EncodeUnary(uiIndex);
				uiIndex = ForwardGolombRiceIndexMapping(n_iMVx[y][x]);
				EncodeUnary(uiIndex);
			}
			if (MVType == 'p') {
				uiIndex = ForwardGolombRiceIndexMapping(p_iMVy[y][x]);
				EncodeUnary(uiIndex);
				uiIndex = ForwardGolombRiceIndexMapping(p_iMVx[y][x]);
				EncodeUnary(uiIndex);
			}
         }
    }
...
void IEncoder::GetPredError()
{
	...
	if (MVType == 'p') {
		...
		for (y = 0; y < _h; y += BLOCKSIZE) {
			for (x = 0; x < _w; x += BLOCKSIZE) {
				GetBlockPredError(_ydata, _ydataRef, y, x, p_iMVy[y / MBSIZE][x / MBSIZE], p_iMVx[y / MBSIZE][x / MBSIZE]);
			}
		}

		for (y = 0; y < _h2; y += BLOCKSIZE) {
			for (x = 0; x < _w2; x += BLOCKSIZE) {
				GetBlockPredError(_udata, _udataRef, y, x, p_iMVy[y / MBSIZEUV][x / MBSIZEUV] / 2, p_iMVx[y / MBSIZEUV][x / MBSIZEUV] / 2);
			}
		}

		for (y = 0; y < _h2; y += BLOCKSIZE) {
			for (x = 0; x < _w2; x += BLOCKSIZE) {
				GetBlockPredError(_vdata, _vdataRef, y, x, p_iMVy[y / MBSIZEUV][x / MBSIZEUV] / 2, p_iMVx[y / MBSIZEUV][x / MBSIZEUV] / 2);
			}
		}
	...
	if (MVType == 'n') {
		...
		for (y = 0; y < _h; y += BLOCKSIZE) {
			for (x = 0; x < _w; x += BLOCKSIZE) {
				GetBlockPredError(_ydata, _ydataRef, y, x, n_iMVy[y / MBSIZE][x / MBSIZE], n_iMVx[y / MBSIZE][x / MBSIZE]);
			}
		}

		for (y = 0; y < _h2; y += BLOCKSIZE) {
			for (x = 0; x < _w2; x += BLOCKSIZE) {
				GetBlockPredError(_udata, _udataRef, y, x, n_iMVy[y / MBSIZEUV][x / MBSIZEUV] / 2, n_iMVx[y / MBSIZEUV][x / MBSIZEUV] / 2);
			}
		}

		for (y = 0; y < _h2; y += BLOCKSIZE) {
			for (x = 0; x < _w2; x += BLOCKSIZE) {
				GetBlockPredError(_vdata, _vdataRef, y, x, n_iMVy[y / MBSIZEUV][x / MBSIZEUV] / 2, n_iMVx[y / MBSIZEUV][x / MBSIZEUV] / 2);
			}
		}
...
// ryadh & mohammad code
void IEncoder::predict_B(float **pfCurrFrame, float **pfRefFrame, float **pfNewFrame){
    ...
    for(x=0; x<_w; x+= window )
    {
        for( y=0; y<_h; y+= window )
        {
            
            
            
            if(sad_p[count] < sad_n[count]) // previous
            {
                for( i=0; i< window; i++)
                {
                    for( j=0; j< window; j++)
                    {
                        //int tempy=p_iMVy[i];
                       pfNewFrame[y + i][x + j] = pfCurrFrame[y + i][x + j] + pfRefFrame[y + i + *p_iMVy[i]][x + j + *p_iMVx[j]];
            ...
            else // new
            {
                for(i=0; i<window ; i++)
                {
                        ...
                        
                    {
                      ...
                      pfNewFrame[y + i][x + j] = pfCurrFrame[y + i][x + j] + pfRefFrame[y + i + *n_iMVy[i]][x + j + *n_iMVx[j]];
...
void IEncoder:: hireacrical_prediction()
{
	cout << "in hierarchal" << endl;
	...
	for(int i = 1; i<=16; i++){
		...
		f[i] = new float *[_w * _h * 3 / 2];// what is 3/2??
		f_new[i] = new float *[_w * _h * 3 / 2];
		U[i] = new float *[(_w * _h * 3 / 2)/2];
		V[i] = new float *[(_w * _h * 3 / 2)/2];
		
	}
	
	unsigned len = _w * _h, i;
	for (i = 0; i < len; i++) {
		f[i] = (float)(pcImgBuf[i]);
	}

	pcImgBuf += len;
	for (i = 0; i < len / 4; i++) {
		U[i] = (float)(pcImgBuf[i]);
	}

	pcImgBuf += len / 4;
	for (i = 0; i < len / 4; i++) {
		V[i] = (float)(pcImgBuf[i]);
	}
	
	
	
    fread(f[0],_w * _h,sizeof(float),fr);
	fread(U[0],_w * _h>>2,sizeof(float),fr);
	fread(V[0],_w * _h>>2,sizeof(float),fr);
	fwrite(f[0],_w * _h,sizeof(float),fw);
	fwrite(U[0],_w * _h>>2,sizeof(float),fw);
	fwrite(V[0],_w * _h>>2,sizeof(float),fw);
	*/
	for(int a = 1; a<=17; a++ ){
		
		/*for(int j = 1; j<=15; j++){
			
		fread(f[j],_w * _h,sizeof(float),fr);
		fread(U[j],_w * _h>>2,sizeof(float),fr);
		fread(V[j],_w * _h>>2,sizeof(float),fr);
			
		}*/
		
		
		for(int y = 0; y < _h; y += MBSIZE) {
		  
			for(int x = 0; x < _w; x += MBSIZE) {
			
				MVType = 'p';//setting previous buffer for MBMotionEst
				MBMotionEst(f, f[6][], y, x);
				EncodeMV();
				GetPredError();//using the mv
				MVType = 'n';//setting next buffer for MBMotionEst
				MBMotionEst(f[12], f[6], y, x);
				EncodeMV();
				GetPredError();//using the mv
				predict_B(f[0], f[12], f_new[6]);
				cout << "predicted f[0]" <<f[0]<<" and f[6]"<<f[6]<<"to "<<f_new[6]<< endl;

				MVType = 'p';	
				MBMotionEst(f[0],f[3],y,x);
				EncodeMV();
				GetPredError();//using the mv
				MVType = 'n';
				MBMotionEst(f_new[6],f[3],y,x);
				EncodeMV();
				GetPredError();//using the mv
				predict_B(f[0],f_new[6],f_new[3]);

				MVType = 'p';
				MBMotionEst(f_new[6],f[9],y,x);
				EncodeMV();
				GetPredError();//using the mv
				MVType = 'n';
				MBMotionEst(f[12],f[9],y,x);
				EncodeMV();
				GetPredError();//using the mv
				predict_B(f_new[6],f[12],f_new[9]);
				
				//MVType = 'n';
				//MBMotionEst(f[12],f[15],y,x);
				//predict_P(f[12],f_new[15]);

				MVType = 'p';
				MBMotionEst(f[0],f[1],y,x);
				EncodeMV();
				GetPredError();//using the mv
				MVType = 'n';
				MBMotionEst(f_new[3],f[1],y,x);
				EncodeMV();
				GetPredError();//using the mv
				predict_B(f[0],f_new[3],f_new[1]);

				MVType = 'p';		
				MBMotionEst(f[0],f[2],y,x);
				EncodeMV();
				GetPredError();//using the mv
				MVType = 'n';
				MBMotionEst(f_new[3],f[2],y,x);
				EncodeMV();
				GetPredError();//using the mv
				predict_B(f[0],f_new[3],f_new[2]);

				MVType = 'p';
				MBMotionEst(f_new[3],f[4],y,x);
				EncodeMV();
				GetPredError();//using the mv
				MVType = 'n';
				MBMotionEst(f_new[6],f[4],y,x);
				EncodeMV();
				GetPredError();//using the mv
				predict_B(f_new[3],f_new[6],f_new[4]);

				MVType = 'p';	
				MBMotionEst(f_new[3],f[5],y,x);
				EncodeMV();
				GetPredError();//using the mv
				MVType = 'n';
				MBMotionEst(f_new[6],f[5],y,x);
				EncodeMV();
				GetPredError();//using the mv
				predict_B(f_new[3],f_new[6],f_new[5]);

				MVType = 'p';
				MBMotionEst(f_new[6],f[7],y,x);
				EncodeMV();
				GetPredError();//using the mv
				MVType = 'n';
				MBMotionEst(f_new[9],f[7],y,x);
				EncodeMV();
				GetPredError();//using the mv
				predict_B(f_new[6],f_new[9],f_new[7]);

				MVType = 'p';	
				MBMotionEst(f_new[6],f[8],y,x);
				EncodeMV();
				GetPredError();//using the mv
				MVType = 'n';
				MBMotionEst(f_new[9],f[8],y,x);
				EncodeMV();
				GetPredError();//using the mv
				predict_B(f_new[6],f_new[9],f_new[8]);

				MVType = 'p';
				MBMotionEst(f_new[9],f[10],y,x);
				EncodeMV();
				GetPredError();//using the mv
				MVType = 'n';
				MBMotionEst(f[12],f[10],y,x);
				EncodeMV();
				GetPredError();//using the mv
				predict_B(f_new[9],f[12],f_new[10]);

				MVType = 'p';	
				MBMotionEst(f_new[9],f[11],y,x);
				EncodeMV();
				GetPredError();//using the mv
				MBMotionEst(f[12],f[11],y,x);
				EncodeMV();
				GetPredError();//using the mv
				predict_B(f_new[9],f[12],f_new[11]);

				MVType = 'p';
				MBMotionEst(f[12],f[13],y,x);
				EncodeMV();
				GetPredError();//using the mv
				MVType = 'n';
				MBMotionEst(f_new[15],f[13],y,x);
				EncodeMV();
				GetPredError();//using the mv
				predict_B(f[12],f_new[15],f_new[13]);

				MVType = 'p';	
				MBMotionEst(f[12],f[14],y,x);
				EncodeMV();
				GetPredError();//using the mv
				MVType = 'n';
				MBMotionEst(f_new[15],f[14],y,x);
				EncodeMV();
				GetPredError();//using the mv
				predict_B(f[12],f_new[15],f_new[14]);
	...
	/*for(int k = 1; k<=15; k++){
		...
		fwrite(f_new[k],_w * _h,sizeof(float),fr);
		fwrite(U[k],_w * _h>>2,sizeof(float),fr);
		fwrite(V[k],_w * _h>>2,sizeof(float),fr);
			
		}
	   
	   
	   	memcpy(f[0],f[15],_w * _h);*/
		cout<< "Number of GOP is : "<< a << endl;
